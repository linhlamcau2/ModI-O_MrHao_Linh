...
#include "vendor/mesh/RD_Switch/RD_Switch.h"
#endif
...
extern void RD_SetSwitchToggle(uint8_t Light_index_set);
//#warning "TEST count K9B"
//uint32_t count_k9B_mess =0;
//----------------------- handle BLE event ---------------------------------------------
...
//	static uint32_t count_k9B_mess =0;
...
//				uart_CSend(UART_TempSend1);
//				uart_CSend("\n");
//				sprintf(UART_TempSend1,"infor: Vid:%4x- Farme %2x--%x--%x--%x",KP9_DataRec->Vid, KP9_DataRec->frame, KP9_DataRec->type_device, KP9_DataRec->key, KP9_DataRec->Counter);
//				uart_CSend(UART_TempSend1);
//				uart_CSend("\n");
//				#warning "TEST count K9B"
//				count_k9B_mess++;
...
//				RD_MessK9BHc_Press(count_k9B_mess/10, 0, 0,0);
				...
				if((0 == scanPressHCStt) && ((key_buff & 0x80) != 0x80))
				{
					/*------Check control onOff-----------*/
					#if(K9B_RD_MESS_EN)
						if(K9B_BLE_CID_RD == KP9_DataRec->Vid){
							//RD_K9B_ScanOnOffV2(MacK9B_Buff, key_buff, signature_last, KP9_DataRec->signature);
							RD_K9B_ScanLocal(MacK9B_Buff, key_buff, signature_last, KP9_DataRec->signature, KP9_DataRec->type_device);
						}else{
							RD_K9B_ScanLocalV1(MacK9B_Buff, key_buff, signature_last, KP9_DataRec->signature, KP9_DataRec->type_device);
...
//	test_simu_io_user_define_proc();
	...
	#if !DU_LPN_EN
	proc_ui();
//	proc_led(); //RD_EDIT: proc_led
	#if(FACTORY_RESET_BY_POW_EN == 1)
		factory_reset_cnt_check(); //RD_EDIT: factory_reset_cnt_check
	#else
	//	factory_reset_cnt_check();
	#endif
	#endif
