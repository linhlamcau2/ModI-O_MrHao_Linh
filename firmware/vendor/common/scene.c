...
extern void RD_Sw_SingleBlinkSet(uint8_t ID_Led, uint16_t cycle, uint16_t countToggle);
int mesh_cmd_sig_scene_set(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	if(cb_par->op_rsp != STATUS_NONE){
		RD_Sw_SingleBlinkSet(cb_par->adr_dst - ele_adr_primary+1, 5, 300); // RD_EDIT: Blink Group
