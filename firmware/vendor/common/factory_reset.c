...
// RD_EDIT: increase_reset_cnt

extern void RD_Switchx4_GW_FacReset();
extern void RD_SwitchAc4Ch_BlinkSet(uint8_t cycle, uint16_t countToggle);
void increase_reset_cnt ()
{
	...
	if(reset_cnt == 10) // time 4, confirm reset
	{
		RD_SwitchAc4Ch_BlinkSet(7, 150);
	}
	if(reset_cnt == 12)
	{
		//RD_Switchx4_GW_FacReset();
		kick_out(0);
	}
...
void kick_out(int led_en){
    ...
    #if !WIN32
    if(led_en){
        //show_factory_reset();  //RD_EDIT: show_factory_reset
    }
    start_reboot();
    #endif
#endif
